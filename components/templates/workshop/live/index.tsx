...
const LiveWorkshopTemplate = () => {
	...
	return activeConnection && !error && !isJoining ? (
		<div className="fixed top-[10vh] w-screen h-screen">
			{/* Left Participants Pane */}
			<div
				className={classNames(
					"z-30 absolute top-0 right-0 h-full flex justify-center items-center duration-300",
					showParticipants ? "w-[80vw] xs:w-[55vw] sm:w-[50vw] lg:w-[30vw]" : "w-0",
				)}>
				<div
					className={`${
						showParticipants ? "hidden" : "animate__animated animate__slideInLeft"
					} right-0 absolute`}>
					<div
						onClick={() => setShowParticipants(!showParticipants)}
						className={`duration-300 bg-white group p-5 rounded-tl-full rounded-bl-full cursor-pointer select-none pt-5`}>
						<div className="flex items-center gap-2 w-full">
							...
							{!showParticipants && (
								<span className="md:flex w-full hidden text-xs text-[#A3A6A7] group-hover:text-black duration-300 whitespace-nowrap">
									All Participants
								</span>
							)}
						</div>
				...
				<div
					className={`z-30 absolute top-0 w-full h-full flex items-center animate__animated animate__fastest ${
			...
			<br />
			{/* Conference Section */}
			<div className="relative min-w-screen h-[65dvh] md:h-[70dvh] z-20 flex justify-center items-center w-full mx-auto">
...
const LiveWorkshopTopSection: FC<{
	...
	<div className="flex sm:flex-row flex-col gap-5 justify-end sm:justify-between sm:item-center sm:px-10 lg:px-0 w-full">

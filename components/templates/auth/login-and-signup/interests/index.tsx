...
const OnboardingInterestsTemplate = () => {
	...
	const handleSubmit = () => {
		if (selectedInterests.length > 0) {
			...
			setLoading(false);
			toast.error("Please select at least one interest", ToastDefaultOptions({ id: "onboarding_interest_pop" }));
		}
	};
										...
										<div className="grid gap-4 bg-white items-center shadow p-1.5 px-3 rounded">
											<div className="flex justify-between item-center">
												...
												<div
													...
													onClick={() => toggleInterest(int.title)}
													style={{ cursor: "pointer" }}>
													<svg width="21" height="20" viewBox="0 0 11 10" fill="none">
														{selectedInterests.includes(int.title) ? (
															<path
											...
											<div className="text-sm text-[#094B10]">{int.title}</div>
										</div>
									</div>
								))}
							</div>

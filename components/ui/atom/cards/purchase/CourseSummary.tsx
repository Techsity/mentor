...
const CourseSummary = (course: ICourse) => {
	return (
		<>
			<div className="sticky h-full py-3 xl:py-8 2xl:py-12 gap-4 w-full bottom-0 text-white bg-[#70C5A1] grid sm:grid-cols-2 items-center lg:px-20 sm:px-12 px-6">
				<div className="grid gap-2">
					<span className="">Course Summary</span>
					<h1
						className="md:text-2xl 2xl:text-4xl font-thin"
					...
					<div className="my-2 sm:my-4 flex items-center gap-2">
						...
						<div className="grid items-center max-w-sm font-[300] text-sm gap-1">
							<h1 className="font-semibold text-lg flex item-center gap-2">
								{course.mentor.name}
								{course.mentor.verified ? (
									<svg
										width="15"
										height="15"
										viewBox="0 0 15 15"
										fill="none"
										className="mt-1.5 bg-zinc-50 rounded-full overflow-hidden flex justify-center items-center">
										<path
							...
							<p className="">{course.mentor.jobTitle} </p>
							<p className="flex gap-1 items-center">
								{/* {formatFollowersCount(course.mentor.courses.length)}{" "} */}
								20 Courses |{" "}
								{formatFollowersCount(course.mentor.sessions)}{" "}
								Sessions |{" "}
								{formatFollowersCount(
									course.mentor.mentees.length,
								)}{" "}
								Mentee |{" "}
								{formatFollowersCount(
									course.mentor.subscribers,
								)}{" "}
								Subscribers
							</p>
						</div>

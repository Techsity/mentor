...
import { IMentor, IMentorEducation } from "../../../../../interfaces/mentor.interface";

const MentorEducationCard = ({ education, className }: { education: IMentorEducation; className?: string }) => {
	return (
		<div className={classNames("border border-[#70C5A1] text-sm p-4 w-full ", className)}>
			<div className="flex justify-between gap-6 items-center mb-4">
				<div className="flex items-start gap-5">
					<div className="">
						<h1 className="font-[500]">{education.course_of_study}</h1>
						<p className="font-[300]">{education.school} </p>
					</div>
					<div className="flex items-center gap-2">
						{new Date(education.from_year).getFullYear()} - {new Date(education.to_year).getFullYear()}
					</div>
				</div>

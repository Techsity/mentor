...
const DashboardSearchbar = ({
	...
	return (
		<form
			...
			className="backdrop-blur-md border border-white rounded-full items-center flex overflow-hidden"
		>
			<input
				onChange={(e) => setSearchQuery(e.target.value)}
				type="text"
				name="dashboard_searchbar"
				id="dashboard_searchbar"
				placeholder="Looking for a topic, course, event, mentor?"
				className="bg-transparent flex-grow px-4 p-3 focus:ring-0 outline-none placeholder:text-sm"
			/>
			<button type="submit" className="right-5 top-3 cursor-pointer pr-5">

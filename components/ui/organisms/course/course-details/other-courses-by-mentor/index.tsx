...
const OtherCoursesByMentor = ({
	...
	return mentorState && mentorCourses && mentorCourses?.length > 0 ? (
		<div className="px-5 sm:px-10 lg:pl-20 mt-10 py-3">
			...
			<div className="flex flex-row items-center overflow-x-scroll gap-3 hide-scroll-bar py-10 relative min-h-[60dvh]">
				...
				{!mentorProfile && (
					<div className="sticky right-0 z-20 w-52 p-3 pr-0 h-full backdrop-blur-sm bg-white/30">
						<div
							onClick={() => router.push(`/mentors/${mentorState.id}`)}
							className="w-full h-full bg-[#70C5A1] p-5 md:p-10 flex flex-col items-start justify-center text-white cursor-pointer select-none">

...
import { useAllUsers } from "../../../../../hooks/admin/admin-resources";
...
const AdminUsersManagement = ({ serverQuery }: { serverQuery: ParsedUrlQuery }) => {
	const { changeOrder, order, paginate, currentFilter, loading, changeCountryFilter, users, currentPage } =
		useAllUsers({
			serverQuery,
		});
	...
	return (
		<div className="relative">
					...
					{loading ? (
						Array.from({ length: 6 }).map((_, index) => {
							return (
								<>
									<br />
									<TableItem loading={loading} key={index} {...{ user: {} }} />
								</>
							);
						})
					) : (
							...
							{users.length >= 1 ? (
								users.map((user, index) => {
									return (
										<>
											<br />
											<TableItem {...{ user, key: index, loading }} />
										</>
									);
								})
			...
			<Pagination {...{ array: users, currentPage, loading, paginate }} />
		</div>
	);
};

const TableItem = ({ user, loading }: { user: Partial<IUser>; loading?: boolean }) => {
	const router = useRouter();
	const country = countries.find(
		(c) =>
			c.countryCode.toLowerCase() === user.country?.toLowerCase() ||
			c.label.toLowerCase() === user.country?.toLowerCase(),
	) as SelectedCountry;
			...
			<td
				...
				onClick={() => {
					router.push({
						...
						query: { tab: "users", mentorId: user.id },
					});
				}}>
				{loading && (
